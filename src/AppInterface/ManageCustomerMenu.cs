...
namespace SecretGarden.OrderSystem.AppInterface{
	class ManageCustomerMenu : Window{
		...
		public ManageCustomerMenu():base("Manage Customers", 2, 1, 34, 13, ConsoleColor.Black){
			...
			this.orders = new MenuList(this, "orders", 2, 5, 30, 6, ConsoleColor.White, ConsoleColor.Black, listItems);
				...
				switch(focus_status){
					...
					case 4:
						...
						if (r_list == ConsoleKey.UpArrow) focus_status = 3;
							...
							ManageCustomerAccountMenu manageCustomerAccountMenu = new ManageCustomerAccountMenu(
								Customer.fetch_customer(listIDs[orders.Index])
							);
							manageCustomerAccountMenu.focus();
							orders.Items = listItems;
						}
						continue;
				}
